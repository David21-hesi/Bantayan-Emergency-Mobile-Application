...
namespace BantayanEmergencyApp
{
    ...
    public partial class LGUNotificationsPage : ContentPage
    {
        ...
        public LGUNotificationsPage(string email)
        {
            InitializeComponent();
            loggedInUserEmail = email.Replace(".", "_"); // Firebase doesn't allow "." in keys
            NotificationsAuthoritiesListView.ItemsSource = filteredNotificationsList;
            LoadNotifications();
        }
        ...
        private async void LoadNotifications()
        {
            try
            {
                ...
                var deletedNotifications = await firebaseClient
                    .Child("DeletedNotifications")
                    .Child(loggedInUserEmail)
                    .OnceAsync<object>();

                var deletedNotificationIds = deletedNotifications.Select(d => d.Key).ToList();
                ...
                foreach (var item in notifications)
                {
                    ...
                    notification.NotificationId = item.Key; // Store Firebase key

                    // Exclude notifications deleted by this user
                    if ((notification.RecipientGroup == "Authorities" || notification.RecipientGroup == "All Users") &&
        ...
        {
            FilterNotifications(e.NewTextValue);
        }
        ...
        private void FilterNotifications(string searchText)
        {
            ...
            foreach (var item in notificationsList.Where(n => n.Message.ToLower().Contains(searchText)))
            {
                filteredNotificationsList.Add(item);
            }
        }
